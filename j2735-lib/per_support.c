...
aper_get_align(asn_per_data_t *pd) {
	...
	if(pd->nboff & 0x7) {
		ASN_DEBUG("Aligning %zu bits", 8 - (pd->nboff & 0x7));
...
int aper_put_align(asn_per_outp_t *po) {
	...
	if(po->nboff & 0x7) {
		ASN_DEBUG("Aligning %zd bits", 8 - (po->nboff & 0x7));
...
uper_put_length(asn_per_outp_t *po, size_t length) {
	...
	ASN_DEBUG("UPER put length %zd", length);
...
aper_put_length(asn_per_outp_t *po, int range, size_t length) {
	...
	ASN_DEBUG("APER put length %zd with range %d", length, range);
