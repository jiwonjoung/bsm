		...
		if(ct->range_bits >= 0) {
			...
			if(specs && specs->field_unsigned) {
				...
				if(uper_get_constrained_whole_number(pd,
					...
					ASN__DECODE_STARVED;
				ASN_DEBUG("Got value %lu + low %"PRId64"",
		...
		if(ct->range_bits >= 0) {
			if (ct->range_bits > 16) {
				...
				if((specs && specs->field_unsigned)
					...
					ASN__DECODE_FAILED;
				ASN_DEBUG("Got value %"PRId64" + low %"PRId64"",
						  value, ct->lower_bound);
			} else {
				long value = 0;
				if (ct->range_bits < 8) {
				...
				if((specs && specs->field_unsigned)
					...
					ASN__DECODE_FAILED;
					ASN_DEBUG("Got value %ld + low %"PRId64"",
							  value, ct->lower_bound);
			}
			return rval;
		} else {
...
INTEGER_encode_uper(asn_TYPE_descriptor_t *td,
	...
	if(ct) {
		...
		if(specs && specs->field_unsigned) {
			...
			ASN_DEBUG("Value %lu (%02x/%d) lb %"PRId64" ub %"PRId64" %s",
			...
			ASN_DEBUG("Value %ld (%02x/%d) lb %"PRId64" ub %"PRId64" %s",
	...
	if(ct && ct->range_bits >= 0) {
		/* #11.5.6 -> #11.3 */
		ASN_DEBUG("Encoding integer %ld (%"PRId64") with range %d bits",
	...
	if(ct && ct->lower_bound) {
		ASN_DEBUG("Adjust lower bound to %"PRId64"", ct->lower_bound);
...
INTEGER_encode_aper(asn_TYPE_descriptor_t *td,
	...
	if(ct) {
		...
		if(specs && specs->field_unsigned) {
			...
			ASN_DEBUG("Value %"PRIu64" (%02x/%d) lb %"PRId64" ub %"PRId64" %s",
								uval, st->buf[0], st->size,
								ct->lower_bound, ct->upper_bound,
								inext ? "ext" : "fix");
						value = uval;
		} else {
			if(asn_INTEGER2int64(st, &value)) ASN__ENCODE_FAILED;
			/* Check proper range */
			if(ct->flags & APC_SEMI_CONSTRAINED) {
				if(value < ct->lower_bound)
					inext = 1;
			...
			ASN_DEBUG("Value %"PRId64" (%02x/%d) lb %"PRId64" ub %"PRId64" %s",
					  value, st->buf[0], st->size,
					  ct->lower_bound, ct->upper_bound,
					  inext ? "ext" : "fix");
		}
	...
	if(ct && ct->range_bits >= 0) {
		/* #10.5.6 */
		ASN_DEBUG("Encoding integer %"PRId64" (%"PRId64") with range %d bits",
			value, value - ct->lower_bound, ct->range_bits);
