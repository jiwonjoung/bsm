...
OCTET_STRING_per_get_characters(asn_per_data_t *po, uint8_t *buf,
	...
	ASN_DEBUG("Expanding %d characters into (%"PRId64"..%"PRId64"):%d",
	...
	for(; buf < end; buf += bpc) {
		...
		if(code < 0) return -1;	/* WMORE */
		if(ch > ub) {
			ASN_DEBUG("Code %d is out of range (%"PRId64"..%"PRId64")",
				ch, lb, ub);
			return 1;	/* FATAL */
		}
		switch(bpc) {
...
OCTET_STRING_per_put_characters(asn_per_outp_t *po, const uint8_t *buf,
	...
	ASN_DEBUG("Squeezing %d characters into (%"PRId64"..%"PRId64"):%d (%d bpc)",
...
OCTET_STRING_decode_uper(asn_codec_ctx_t *opt_codec_ctx,
	...
	ASN_DEBUG("PER Decoding %s size %"PRId64" .. %"PRId64" bits %d",
	...
	if(csiz->effective_bits == 0) {
		int ret;
		if(bpc) {
			ASN_DEBUG("Encoding OCTET STRING size %"PRId64"",
				csiz->upper_bound);
			...
			ASN_DEBUG("Encoding BIT STRING size %"PRId64"",
				csiz->upper_bound);
...
// 			unit_bits = cval->range_bits;
	...
	if(csiz->effective_bits == 0) {
		...
		if(bpc) {
			ASN_DEBUG("Decoding OCTET STRING size %"PRId64"",
				csiz->upper_bound);
			ret = OCTET_STRING_per_get_characters(pd, st->buf,
				csiz->upper_bound, bpc, unit_bits,
				cval->lower_bound, cval->upper_bound, pc);
			...
			ASN_DEBUG("Decoding BIT STRING size %"PRId64"",
				csiz->upper_bound);
			ret = per_get_many_bits(pd, st->buf, 0,
					    unit_bits * csiz->upper_bound);
		}
...
OCTET_STRING_encode_uper(asn_TYPE_descriptor_t *td,
	...
	/* X.691, #16.7: long fixed length encoding (up to 64K octets) */
	if(csiz->effective_bits >= 0) {
		ASN_DEBUG("Encoding %d bytes (%"PRId64"), length in %d bits",
